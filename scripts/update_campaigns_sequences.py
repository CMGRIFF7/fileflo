"""
update_campaigns_sequences.py
Updates the 3 new campaigns with full multi-variant sequences and custom variables.
"""
...
CAMPAIGN_IDS = {
    "FMCSA CSA Score Alert": "53b6c3d3-e4d6-4067-be6e-87542f1be716",
    "FMCSA Safety Rating Alert": "a4f77f6e-3033-4db0-8b1e-f128b5bfbdd6",
    "FMCSA Violation History Pattern": "8f41f5d5-a3f8-41f2-9143-b01bfa5bdc8b"
}
    ...
    for line in resp.iter_lines():
        if line:
            ...
            if decoded.startswith("data:"):
                try:
                    result_text = json.loads(decoded[5:].strip())
                except Exception:
                    pass
    return result_text
...
updates = [
    {
        "campaign_id": CAMPAIGN_IDS["FMCSA CSA Score Alert"],
    ...
    {
        "campaign_id": CAMPAIGN_IDS["FMCSA Safety Rating Alert"],
    ...
    {
        "campaign_id": CAMPAIGN_IDS["FMCSA Violation History Pattern"],
...
for i, u in enumerate(updates):
    print(f"\nUpdating: {u['name']}...")
    result = call_tool(session_id, "update_campaign", {
        "params": {
            "campaign_id": u["campaign_id"],
            "sequences": u["sequences"],
            "custom_variables": u["custom_variables"],
            "open_tracking": False,
            "link_tracking": False,
            "text_only": True,
            "stop_on_reply": True,
    ...
    if result:
        ...
        # Show just the id and status from the response
        try:
            data = json.loads(text)
            print(f"  ID: {data.get('id')} | Status: {data.get('status')} | Sequences: {len(data.get('sequences', [{}])[0].get('steps', []))} steps")
        except Exception:
